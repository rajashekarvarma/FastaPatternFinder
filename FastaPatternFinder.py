...
### CREATE DATAFRAME
        ...
        dfp[['id','location','Pre_sequence(PXX)']] = dfp.pp.str.split("_",expand=True,)
        dfp.drop(['pp'], axis=1, inplace = True)
        dfm=pd.DataFrame.from_dict(mm)
        dfm[['id','location','Med_sequence(PXP)']] = dfm.mm.str.split("_",expand=True,)
        dfm.drop(['mm'], axis=1, inplace = True)
        dfs=pd.DataFrame.from_dict(ss)
        dfs[['id','location','Suc_sequence(XXP)']] = dfs.ss.str.split("_",expand=True,)
        dfs.drop(['ss'], axis=1, inplace = True)
        dfa=pd.merge(pd.merge(dfp,dfm,on=['id','location'],how='outer'),dfs,on=['id','location'],how='outer')
        ...
        dfa.sort_values(by=['id','location'], inplace=True)
