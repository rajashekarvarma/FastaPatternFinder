...
def seqParser(fasta_sequences):
        ...
        dfp[['id','location','Pre_sequence']] = dfp.pp.str.split("_",expand=True,)
        dfp.drop(['pp'], axis=1, inplace = True)
        dfm=pd.DataFrame.from_dict(mm)
        dfm[['id','location','Med_sequence']] = dfm.mm.str.split("_",expand=True,)
        dfm.drop(['mm'], axis=1, inplace = True)
        dfs=pd.DataFrame.from_dict(ss)
        dfs[['id','location','Suc_sequence']] = dfs.ss.str.split("_",expand=True,)
